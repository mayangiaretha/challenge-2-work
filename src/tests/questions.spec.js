...
describe('Test the questions feature',  function() {
  ...
  it('Should create an answer', async () => {
    const response = await BaseTest.post(`questions/${questionId}/answers`).send({
  ...
  it('Should get an answer to a question', async () => {
    const response = await BaseTest.get(`questions/3d104a9f-bb18-4ded-a1b5-68b62f15de0a/answers`)
    expect(response.status).to.equal(200);
    expect(response.body[0].questionId).to.equal('3d104a9f-bb18-4ded-a1b5-68b62f15de0a')
    });
  ...
  it('Should get an error if we try  to get an answer to a question without an answer', async () => {
    ...
    expect(response.status).to.equal(400);
    expect(response.body.message).to.equal('Answer to this question does not exist')
  });
  ...
  it('Should fail to create an answer if an invalid id is provided', async () => {
    ...
    expect(response.status).to.equal(400);
    expect(response.body).to.include({error: "question does not exist please check id"})
  });
});
